...
static EVP_PKEY* rsa_pem_to_evp_key(const char* pem, size_t len) {
  BIO* bio = BIO_new_mem_buf(pem, (int)len);
  if (!bio) return NULL;
  ...
  if (key) { BIO_free(bio); return key; }

  BIO_reset(bio);

  // Try RSA public key (PKCS#8 SubjectPublicKeyInfo)
...
DECLARE_MODULE_METHOD(crypto_random_bytes) {
  ...
  if (!IS_NUMBER(args[0]))
    RETURN_ERROR("random_bytes() expects a number");
  int n = (int)AS_NUMBER(args[0]);
  if (n <= 0 || n > 65536)
    RETURN_ERROR("random_bytes(): length must be 1..65536");
  unsigned char* buf = malloc(n);
  if (!buf)
    RETURN_ERROR("random_bytes(): out of memory");
...
DECLARE_MODULE_METHOD(crypto_aes_gcm_encrypt) {
  ...
  if (!ctx)
    RETURN_ERROR("aes_gcm_encrypt(): context allocation failed");
  ...
  if (!out) {
    EVP_CIPHER_CTX_free(ctx);
    RETURN_ERROR("aes_gcm_encrypt(): out of memory");
  }
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("aes_gcm_encrypt(): encryption failed");
  }
...
DECLARE_MODULE_METHOD(crypto_aes_gcm_decrypt) {
  ...
  if (ctl < 16)
    RETURN_ERROR("aes_gcm_decrypt(): ciphertext too short (minimum 16 bytes for tag)");
  ...
  EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
  if (!ctx)
    RETURN_ERROR("aes_gcm_decrypt(): context allocation failed");
  ...
  if (!out) {
    EVP_CIPHER_CTX_free(ctx);
    RETURN_ERROR("aes_gcm_decrypt(): out of memory");
  }
...
DECLARE_MODULE_METHOD(crypto_aes_cbc_encrypt) {
  ...
  if (ivl != 16)
    RETURN_ERROR("aes_cbc_encrypt(): iv must be exactly 16 bytes");

  const EVP_CIPHER* cipher = NULL;
  switch (kl) {
    case 16: cipher = EVP_aes_128_cbc();
      break;
    case 24: cipher = EVP_aes_192_cbc();
      break;
    case 32: cipher = EVP_aes_256_cbc();
      break;
  ...
  if (!out)
    RETURN_ERROR("aes_cbc_encrypt(): out of memory");
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("aes_cbc_encrypt(): encryption failed");
  }
  RETURN_BYTES(out, out_len + final_len);
}
...
DECLARE_MODULE_METHOD(crypto_aes_cbc_decrypt) {
  ...
  if (ivl != 16)
    RETURN_ERROR("aes_cbc_decrypt(): iv must be exactly 16 bytes");
  ...
  if (!out)
    RETURN_ERROR("aes_cbc_decrypt(): out of memory");
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("aes_cbc_decrypt(): decryption failed — bad padding or wrong key");
  }
  RETURN_BYTES(out, out_len + final_len);
}
...
DECLARE_MODULE_METHOD(crypto_chacha20_encrypt) {
  ...
  if (key_obj->bytes.count != 32)
    RETURN_ERROR("chacha20_encrypt(): key must be exactly 32 bytes");
  if (iv_obj->bytes.count != 12)
    RETURN_ERROR("chacha20_encrypt(): iv must be exactly 12 bytes");

  int ptl = pt_obj->bytes.count;
  unsigned char* out = malloc(ptl + 16);
  if (!out)
    RETURN_ERROR("chacha20_encrypt(): out of memory");
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("chacha20_encrypt(): encryption failed");
  }
  RETURN_BYTES(out, out_len + final_len + 16);
}
...
DECLARE_MODULE_METHOD(crypto_chacha20_decrypt) {
  ...
  if (key_obj->bytes.count != 32)
    RETURN_ERROR("chacha20_decrypt(): key must be exactly 32 bytes");
  if (iv_obj->bytes.count != 12)
    RETURN_ERROR("chacha20_decrypt(): iv must be exactly 12 bytes");

  int ctl = ct_obj->bytes.count;
  if (ctl < 16)
    RETURN_ERROR("chacha20_decrypt(): ciphertext too short");
  int ptl = ctl - 16;
  ...
  unsigned char* out = malloc(ptl > 0 ? ptl : 1);
  if (!out)
    RETURN_ERROR("chacha20_decrypt(): out of memory");
...
DECLARE_MODULE_METHOD(crypto_rsa_generate) {
  ...
  if (!ctx)
    RETURN_ERROR("rsa_generate(): context failed");
...
DECLARE_MODULE_METHOD(crypto_rsa_encrypt) {
  ENFORCE_ARG_COUNT(rsa.encrypt, 2);
  ENFORCE_ARG_TYPE(rsa.encrypt, 0, IS_STRING);
  ENFORCE_ARG_TYPE(rsa.encrypt, 1, IS_BYTES);
  ...
  if (!pkey)
    RETURN_ERROR("rsa.encrypt(): failed to parse public key PEM");

  EVP_PKEY_CTX* ctx = EVP_PKEY_CTX_new(pkey, NULL);
  EVP_PKEY_free(pkey);
  if (!ctx)
    RETURN_ERROR("rsa.encrypt(): context failed");
  ...
  if (!ok) {
    EVP_PKEY_CTX_free(ctx);
    RETURN_ERROR("rsa.encrypt(): padding configuration failed");
  }
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("rsa.encrypt(): encryption failed");
  }
...
DECLARE_MODULE_METHOD(crypto_rsa_decrypt) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("rsa_decrypt(): failed to parse private key PEM");

  EVP_PKEY_CTX* ctx = EVP_PKEY_CTX_new(pkey, NULL);
  EVP_PKEY_free(pkey);
  if (!ctx)
    RETURN_ERROR("rsa_decrypt(): context failed");
  ...
  if (!ok) {
    EVP_PKEY_CTX_free(ctx);
    RETURN_ERROR("rsa_decrypt(): padding configuration failed");
  }
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("rsa_decrypt(): decryption failed");
  }
  RETURN_BYTES(out, (int)out_len);
}
...
DECLARE_MODULE_METHOD(crypto_rsa_sign) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("rsa_sign(): failed to parse private key PEM");
  ...
  if (!ok) {
    free(sig);
    RETURN_ERROR("rsa_sign(): signing failed");
  }
...
DECLARE_MODULE_METHOD(crypto_rsa_verify) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("rsa_verify(): failed to parse public key PEM");
...
DECLARE_MODULE_METHOD(crypto_ecdsa_generate) {
  ...
  if (strcmp(curve->chars, "P-256") == 0) nid = NID_X9_62_prime256v1;
    ...
    RETURN_ERROR("ecdsa_generate(): curve must be \"P-256\" or \"P-384\"");

  EVP_PKEY* pkey = ecdsa_generate_key(nid);
  if (!pkey)
    RETURN_ERROR("ecdsa_generate(): key generation failed");
  ...
  if (!priv || !pub) {
    free(priv);
    free(pub);
    RETURN_ERROR("ecdsa_generate(): PEM export failed");
  }
...
DECLARE_MODULE_METHOD(crypto_ecdsa_sign) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("ecdsa_sign(): failed to parse private key PEM");
  ...
  if (!ok) {
    free(sig);
    RETURN_ERROR("ecdsa_sign(): signing failed");
  }
  RETURN_BYTES(sig, (int)sig_len);
}
...
DECLARE_MODULE_METHOD(crypto_ecdsa_verify) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("ecdsa_verify(): failed to parse public key PEM");
...
DECLARE_MODULE_METHOD(crypto_ed25519_generate) {
  ...
  if (!ctx)
    RETURN_ERROR("ed25519_generate(): context failed");
  ...
  if (!ok || !pkey) {
    if (pkey) EVP_PKEY_free(pkey);
    RETURN_ERROR("ed25519_generate(): keygen failed");
  }
  ...
  if (!priv || !pub) {
    free(priv);
    free(pub);
    RETURN_ERROR("ed25519_generate(): PEM export failed");
  }
...
DECLARE_MODULE_METHOD(crypto_ed25519_sign) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("ed25519_sign(): failed to parse private key PEM");
  ...
  if (!ok) {
    free(sig);
    RETURN_ERROR("ed25519_sign(): signing failed");
  }
  RETURN_BYTES(sig, (int)sig_len);
}
...
DECLARE_MODULE_METHOD(crypto_ed25519_verify) {
  ...
  EVP_PKEY* pkey = pem_to_evp_key(pem_str->chars, pem_str->length);
  if (!pkey)
    RETURN_ERROR("ed25519_verify(): failed to parse public key PEM");
...
DECLARE_MODULE_METHOD(crypto_x25519_generate) {
  ...
  if (!ctx)
    RETURN_ERROR("x25519_generate(): context failed");
  ...
  if (!ok || !pkey) {
    if (pkey) EVP_PKEY_free(pkey);
    RETURN_ERROR("x25519_generate(): keygen failed");
  }
  ...
  if (!priv || !pub) {
    free(priv);
    free(pub);
    RETURN_ERROR("x25519_generate(): PEM export failed");
  }
...
DECLARE_MODULE_METHOD(crypto_x25519_exchange) {
  ...
  if (!priv_key)
    RETURN_ERROR("x25519_exchange(): failed to parse private key PEM");

  EVP_PKEY* pub_key = pem_to_evp_key(pub_str->chars, pub_str->length);
  if (!pub_key) {
    EVP_PKEY_free(priv_key);
    RETURN_ERROR("x25519_exchange(): failed to parse peer public key PEM");
  }
  ...
  if (!ok) {
    free(secret);
    RETURN_ERROR("x25519_exchange(): key exchange failed");
  }
...
DECLARE_MODULE_METHOD(crypto_argon2_hash) {
  ...
  if (arg_count == 3 && IS_DICT(args[2])) {
    ...
    z_value v;
    if (dict_get_entry(opts, STRING_VAL("t_cost"), &v) && IS_NUMBER(v)) t_cost = (uint32_t)AS_NUMBER(v);
    if (dict_get_entry(opts, STRING_VAL("m_cost"), &v) && IS_NUMBER(v)) m_cost = (uint32_t)AS_NUMBER(v);
    if (dict_get_entry(opts, STRING_VAL("threads"), &v) && IS_NUMBER(v)) threads = (uint32_t)AS_NUMBER(v);
    if (dict_get_entry(opts, STRING_VAL("hash_len"), &v) && IS_NUMBER(v)) hash_len = (uint32_t)AS_NUMBER(v);
  }
  ...
  if (!encoded)
    RETURN_ERROR("argon2_hash(): out of memory");
...
DECLARE_MODULE_METHOD(crypto_hkdf) {
  ...
  if (!IS_BYTES(args[0]))
    RETURN_ERROR("hkdf(): ikm must be bytes");
  if (!IS_BYTES(args[1]))
    RETURN_ERROR("hkdf(): salt must be bytes");
  if (!IS_BYTES(args[2]))
    RETURN_ERROR("hkdf(): info must be bytes");
  if (!IS_NUMBER(args[3]))
    RETURN_ERROR("hkdf(): length must be a number");
  ...
  if (length <= 0 || length > 8160)
    RETURN_ERROR("hkdf(): length must be 1..8160");

  unsigned char* out = malloc(length);
  if (!out)
    RETURN_ERROR("hkdf(): out of memory");
  ...
  if (!ok) {
    free(out);
    RETURN_ERROR("hkdf(): derivation failed");
  }
...
CREATE_MODULE_LOADER(crypto) {
  ...
  static z_module_reg module = {
    ...
    .functions = functions,
    .classes = NULL,
    // .preloader = &z__crypto_module_preloader,
    // .unloader = &z__crypto_module_unloader,
    .preloader = NULL,
    .unloader = NULL,
  };
