...
DECLARE_MODULE_METHOD(crypto_aes_gcm_encrypt) {
  ...
  unsigned char* out = ALLOCATE(unsigned char, ptl + 16);
  ...
  RETURN_T_BYTES(out, out_len + final_len + 16);
}
...
DECLARE_MODULE_METHOD(crypto_aes_gcm_decrypt) {
  ...
  unsigned char* out = ALLOCATE(unsigned char, ptl > 0 ? ptl : 1);
  ...
  RETURN_T_BYTES(out, out_len + final_len);
}
...
DECLARE_MODULE_METHOD(crypto_aes_cbc_encrypt) {
  ...
  unsigned char* out = ALLOCATE(unsigned char, out_buf_len);
...
DECLARE_MODULE_METHOD(crypto_aes_cbc_decrypt) {
  ...
  unsigned char* out = ALLOCATE(unsigned char, ctl);
...
DECLARE_MODULE_METHOD(crypto_rsa_encrypt) {
  ...
  if (!pkey)
    RETURN_ERROR("rsa_encrypt(): failed to parse public key PEM");

  EVP_PKEY_CTX* ctx = EVP_PKEY_CTX_new(pkey, NULL);
  EVP_PKEY_free(pkey);
  if (!ctx)
    RETURN_ERROR("rsa_encrypt(): context failed");
  ...
  if (!ok) {
    EVP_PKEY_CTX_free(ctx);
    RETURN_ERROR("rsa_encrypt(): padding configuration failed");
  }
  ...
  unsigned char* out = ok ? ALLOCATE(unsigned char, out_len) : NULL;
  ok = out && EVP_PKEY_encrypt(ctx, out, &out_len, pt, ptl) > 0;
  EVP_PKEY_CTX_free(ctx);

  if (!ok) {
    FREE(unsigned char, out);
    RETURN_ERROR("rsa_encrypt(): encryption failed");
  }

  RETURN_T_BYTES(out, (int)out_len);
}
...
DECLARE_MODULE_METHOD(crypto_rsa_sign) {
  ...
  unsigned char* sig = ok ? ALLOCATE(unsigned char, sig_len) : NULL;
  ...
  if (!ok) {
    FREE(unsigned char, sig);
    RETURN_ERROR("rsa_sign(): signing failed");
  }

  RETURN_T_BYTES(sig, (int)sig_len);
}
...
DECLARE_MODULE_METHOD(crypto_ecdsa_generate) {
  ...
  z_obj_dict* result = (z_obj_dict*)GC(new_dict(vm));
  dict_add_entry(vm, result, GC_STRING("private_pem"), GC_TT_STRING(priv));
  dict_add_entry(vm, result, GC_STRING("public_pem"), GC_TT_STRING(pub));
  RETURN_OBJ(result);
}
...
DECLARE_MODULE_METHOD(crypto_ed25519_sign) {
  ...
  EVP_MD_CTX* mctx = EVP_MD_CTX_new();
  size_t sig_len = 64;
  unsigned char* sig = ALLOCATE(unsigned char, sig_len);
...
DECLARE_MODULE_METHOD(crypto_x25519_exchange) {
  ...
  unsigned char* secret = ALLOCATE(unsigned char, secret_len);
  ...
  if (!ok) {
    FREE(unsigned char, secret);
    RETURN_ERROR("x25519_exchange(): key exchange failed");
  }

  RETURN_T_BYTES(secret, (int)secret_len);
}
...
DECLARE_MODULE_METHOD(crypto_argon2_hash) {
  ...
  if (return_bytes) {
    ...
    char* encoded = ALLOCATE(char, encoded_len);
...
DECLARE_MODULE_METHOD(crypto_hkdf) {
  ...
  unsigned char* out = ALLOCATE(unsigned char, length);
