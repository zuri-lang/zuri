...
Zuri packages are simply zuri projects published on a Zuri repository. This document highlights the steps and guides to publishing a package to a repository.
...
1. It must be unique. Package names are unique throughout a repository irrespective of who publishes them. 
  ...
  It is advisable to first search the repository you want to publish to (such as [zuri.zurilang.org](zuri.zurilang.org)) for any package already using the name you want to choose. 
...
3. It should not infringe on someone else's trademarks nor be misleading.

Also, a good package names should be usable as a valid variable name.

### The README.md file

One of the files generated by zuri during `zuri init` is the `README.md` file. Before publishing a package, do your best to communicate as much information and/or documentation as you can in this file because this is the file that will be shown to people who come to browse your package on repositories.
...
Everytime you publish a new package or an update to an existing package, zuri _requires_ that the package version changes as well. Following the semantic versioning when changing/updating this version helps keep the ecosystem clean and predictable as well as easy to navigate for other developers.
...
Once you are logged in to your publisher account, simply run the command `zuri publish` from the root of your package and zuri will take care of the rest.
...
- Create a new zuri application in the directory by running `zuri init`.
- Run the command `zuri install <package_name>`.
- Open the file at `app/index.zu` and insert code to import your package before every code in the file. E.g.

```zuri
import <package_name>
echo 'Welcome to Zuri. Magic begins here!'
```

- Run the application (`zuri run`)
